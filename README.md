...
- **认证**: GitLab SSO 登录（支持自签名证书），Mock 登录（开发模式），JWT Token
- **SSO 用户匹配**: 按 GitLab username 匹配已有用户（保留角色权限），不存在则自动创建（viewer 角色）
- **角色**: 预置admin、developer、operator、viewer角色
...
一次操作为多个部署单元创建发布单，共享同一版本号。

- **多DU选择**: 按竖井/系统筛选，多选部署单元
- **版本设置**: 输入新版本，查看各DU当前版本
- **预览确认**: 按DU汇总变更，一键创建多个发布单

### 4. 审批管理

- **待审批列表**: 集中展示所有待审批阶段，支持批量审批
- **审批通过/驳回**: 驳回后整单失败
- **审批备注**: 支持添加备注
- **审批历史**: 查看历史审批记录
...
- **CCWork Webhook**: 按环境配置 webhook URL
- **审批人发现**: 自动查找具有 approve 权限的用户
- **纯文本消息**: 包含 DU 信息、环境、审批链接

## 技术架构

### 前端

- 纯HTML/CSS/JavaScript单页应用（ES Modules），无外部依赖
- 暗色侧边栏设计，SVG图标
- DAG编辑器：SVG画布 + 贝塞尔曲线边 + 拖拽交互
- 页面模块化：发布管理、批量发布、部署单元浏览、审批中心、蓝图编辑、权限管理
- 跨环境配置对比 + LCS 行级 diff 模态框
...
访问 `http://localhost:8080`，首次运行时通过 `/api/init` 创建 admin 用户，或选择 mock 用户登录（alice 自动分配 admin 角色）。
...
| GET | /api/deploy-units | 部署单元列表（DMDB） |
| GET | /api/deploy-units/:code | 部署单元详情 |
| GET | /api/deploy-units/:code/compare | 跨环境配置对比 |
| GET | /api/du-list | 全量DU列表（DevOps API，支持silo/system筛选） |
...
| POST | /api/releases/:id/deprecate | 废弃发布 |
| DELETE | /api/releases/:id | 删除发布（仅admin，7天内废弃的） |
| POST | /api/stages/:stageId/approve | 审批通过（触发DMDB配置推送） |
| POST | /api/stages/:stageId/reject | 审批驳回（整单失败） |
| POST | /api/stages/:stageId/promote | 推进到下一环境 |
| POST | /api/stages/:stageId/retry-push | 重试失败的配置推送 |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | /api/admin/users/batch | 批量创建用户（JSON） |
| GET | /api/admin/users | 用户列表 |
| GET | /api/admin/roles | 角色列表 |
| GET | /api/admin/roles/:roleId | 角色详情 |
| POST | /api/admin/roles | 创建角色 |
| PUT | /api/admin/users/:userId/roles | 设置用户角色 |
| PUT | /api/admin/users/:userId/access | 设置用户访问范围（silo/env） |
| PUT | /api/admin/roles/:roleId/permissions | 设置角色权限 |

### 通知配置
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | /api/admin/notification-config | 获取通知配置 |
| PUT | /api/admin/notification-config | 保存通知配置 |

## 配置

支持 YAML 配置文件 + 环境变量覆盖。优先级：**环境变量 > YAML 文件 > 默认值**。
